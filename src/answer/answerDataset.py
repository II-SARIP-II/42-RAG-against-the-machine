...
class AnswerDataset():
    ...
    def findSearchDatasetResult(self) -> None:
        ...
        searchResult = StudentDetailedSearchResults.model_validate(
            searchDatasetResultsDict
            )
        if searchResult.k > self.k:
            self.k = searchResult.k
        answerDataset = []
        for minimalSearchResults in tqdm(searchResult.search_results,
                                         desc="LLM answering"):
            answerMinimalSearch = Answer(minimalSearchResults.question,
                                         self.k)
