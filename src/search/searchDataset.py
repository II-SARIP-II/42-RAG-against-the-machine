...
from src.models.Result import StudentSearchResultsCompleteSource
...
class SearchDataset():
    ...
    def findAllQuestions(self):
        ...
        question_adapter = TypeAdapter(Union[AnsweredQuestion, UnansweredQuestion])    
        questions = [question_adapter.validate_python(item) for item in raw_list["rag_questions"]]
    ...
    def saveSearchDataset(self):
        ...
        path = str(self.output_path) + '/' + str(self.dataset_path).split('/')[-1]
        os.makedirs(self.output_path, exist_ok=True)
        with open(path, "w", encoding="utf-8") as f:
            json.dump(self.studentSearchResults, f, indent=4, ensure_ascii=False)
