...
class Search():
    ...
    def findSources(self) -> None:
        ...
        if self.chroma:
            ...
            chroma_ids = chroma_result.get("ids", [[]])[0] if chroma_result else []
        ...
        corpus_dict = {str(item.get("chunk_id")): item for item in raw_list if item.get("chunk_id") is not None}

        sources_formatted: List[DetailedSource] = []
        for chunk_id in final_ranked_ids:
            if chunk_id in corpus_dict:
                validated_source = DetailedSource.model_validate(corpus_dict[chunk_id])
    ...
    def rank_fusion(self, bm25_ids, chroma_ids, k_rrf=60):
        score_idx = {}
        
        for i, doc_id in enumerate(bm25_ids):
            rank = i + 1
            score_idx[doc_id] = score_idx.get(doc_id, 0.0) + (1.0 / (k_rrf + rank))
            
        for i, doc_id in enumerate(chroma_ids):
            rank = i + 1
            score_idx[doc_id] = score_idx.get(doc_id, 0.0) + (1.0 / (k_rrf + rank))
            
        sorted_ids = [k for k, v in sorted(score_idx.items(), key=lambda item: item[1], reverse=True)]
        return sorted_ids
